...
describe("dogfood status reporter", () => {
  ...
  it("reports desktop session blockers separately from permission blockers", async () => {
    ...
    expect(status).toMatchObject({
      ...
      nextActions: expect.arrayContaining([
        "Unlock the Mac and keep the display awake before requiring passed Ghostty/Finder Computer Use evidence.",
        ...
        "When desktop preflight passes, rerun packaged product smokes with --require-passed for Ghostty and Finder.",
  ...
  it("does not request skfiy-owned speech evidence for agent Computer Use turns", async () => {
    const { createDogfoodStatus } = await import(pathToFileURL(modulePath).href) as {
      createDogfoodStatus: (
        input: Record<string, unknown>,
        io?: Record<string, unknown>
    ...
    expect(status.nextActions.join("\n")).not.toMatch(/smoke:voice|Speech|Microphone|speech/i);
    expect(io.textFiles[summaryPath]).not.toMatch(/smoke:voice|Speech|Microphone|speech/i);
  });
  ...
  it("reports desktop session blockers from Ghostty/Finder smoke preflight artifacts", async () => {
    const { createDogfoodStatus } = await import(pathToFileURL(modulePath).href) as {
      createDogfoodStatus: (
        input: Record<string, unknown>,
        io?: Record<string, unknown>
  ...
  it("reports missing manifest smoke artifacts without aborting status", async () => {
    ...
    expect(status.localSmoke.artifactResults.finder).toBe("missing");
    expect(status.nextActions).toContain(
      "Regenerate or attach missing smoke artifacts before relying on local readiness: finder (/repo/.skfiy-smoke/finder.json)."
    );
    expect(io.textFiles[summaryPath]).toContain("- finder: missing");
  });
  ...
  it("recommends reposting the current alpha tester assignment packet when it lacks the desktop session preflight", async () => {
    const { createDogfoodStatus } = await import(pathToFileURL(modulePath).href) as {
      createDogfoodStatus: (
        input: Record<string, unknown>,
        io?: Record<string, unknown>
    ...
    const io = createMemoryIo({
      ...
      [trackingIssueUrl]: {
        ...
        labels: ["skfiy", "dogfood"],
        comments: [
          createAssignmentComment("abc123", { includeDesktopSessionPreflight: false })
        ]
      }
    });
    ...
    expect(status).toMatchObject({
      trackingIssue: {
        assignmentComment: {
          ...
          matchingCommentCount: 1,
          reasons: [
            "current skfiy-alpha-abc123 tester assignment packet comment is missing Desktop Session Preflight"
          ]
        }
      },
      nextActions: expect.arrayContaining([
    ...
    expect(io.textFiles[summaryPath]).toContain("- invalid: current skfiy-alpha-abc123 tester assignment packet comment is missing Desktop Session Preflight");
  });

  it("recommends reposting the current alpha tester assignment packet when it lacks the app bundle preflight", async () => {
    const { createDogfoodStatus } = await import(pathToFileURL(modulePath).href) as {
      createDogfoodStatus: (
        input: Record<string, unknown>,
        io?: Record<string, unknown>
    ...
    const io = createMemoryIo({
      ...
      [trackingIssueUrl]: {
        ...
        labels: ["skfiy", "dogfood"],
        comments: [
          createAssignmentComment("abc123", { includeAppBundlePreflight: false })
        ]
      }
    });
    ...
    expect(status).toMatchObject({
      trackingIssue: {
        assignmentComment: {
          ...
          matchingCommentCount: 1,
          reasons: [
            "current skfiy-alpha-abc123 tester assignment packet comment is missing App Bundle Preflight"
          ]
        }
      },
      nextActions: expect.arrayContaining([
    ...
    expect(io.textFiles[summaryPath]).toContain("- invalid: current skfiy-alpha-abc123 tester assignment packet comment is missing App Bundle Preflight");
  });
...
function createTrackingIssueBody(
  ...
  const desktopSessionPreflightLines = options.includeDesktopSessionPreflight === false
    ...
    : [
      ...
      "In strict mode, `dogfood:tester` uses a strict desktop-session preflight before Ghostty/Chrome/Finder/money-run."
    ];
...
function createAssignmentComment(
  ...
  return {
    body: [
      ...
      "",
      options.includeAppBundlePreflight === false ? "" : "## App Bundle Preflight",
      "",
      options.includeDesktopSessionPreflight === false ? "" : "## Desktop Session Preflight",
      "",
