...
describe("window bounds product API", () => {
  it("exposes read-only pet window bounds through main, preload, and renderer types", () => {
    ...
    const rendererTypesPath = path.join(process.cwd(), "src", "renderer", "app-types.ts");

    expect(existsSync(mainPath)).toBe(true);
    expect(existsSync(preloadPath)).toBe(true);
    expect(existsSync(rendererTypesPath)).toBe(true);
    ...
    expect(rendererTypesSource).toContain("getWindowBounds: () => Promise<WindowBounds | null>");
  });
});
