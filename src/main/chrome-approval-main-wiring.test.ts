...
describe("Chrome approval main-process wiring", () => {
  it("records approved Chrome tasks into host policy before execution", () => {
    ...
    expect(source).toContain("Chrome host policy allowed for current turn");
    expect(source).toContain("Chrome host policy blocked this approved task");
    expect(source).toContain("Chrome host policy approval failed");
  });
});
