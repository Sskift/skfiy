...
function renderDashboardScript(): string {
  return [
    ...
    "    target.append(createMetricList(rows.map(sanitizeUserPanelRow)));",
    ...
    "      empty.textContent = sanitizeUserPanelText(emptyText);",
    "      target.append(empty);",
    "    } else {",
    ...
    "        entry.textContent = sanitizeUserPanelText(item);",
    "        list.append(entry);",
    "      }",
    "      target.append(list);",
    "    }",
    ...
    "  }",
    "",
    "  function sanitizeUserPanelRow([label, value]) {",
    "    return [label, sanitizeUserPanelText(value)];",
    "  }",
    "",
    "  function sanitizeUserPanelText(value) {",
    ...
    "    const routeOutcome = readFallbackRouteOutcome(snapshot, turn);",
    "    const assistant = readAssistantState(snapshot, turn, freshness, routeOutcome);",
    "    setUserRows(\"home\", [",
    "      row(\"assistant\", assistant.detail),",
    "      row(\"current task\", turn.command || turn.latestMessage || readFallbackRouteOutcomeDetail(routeOutcome) || \"No active task\"),",
    ...
    "      row(\"next\", readUserNextAction(snapshot, routeOutcome)),",
    ...
    "    const turn = snapshot.currentTurn || {};",
    "    const items = [];",
    "    if (turn.state === \"needs_confirmation\") {",
    "      items.push(`Route confirmation: ${turn.latestMessage || turn.command || \"Review the pending route confirmation.\"}`);",
    "    } else if (turn.approvalState === \"required\" || turn.state === \"approval_required\") {",
    "      items.push(`Computer Use approval: ${turn.risk || \"approval\"}: ${turn.command || turn.latestMessage || \"Review the pending Computer Use action.\"}`);",
    "    }",
    ...
    "  }",
    "",
    "  function readAssistantState(snapshot, turn, freshness, routeOutcome) {",
    ...
    "    if (turn.state === \"approval_required\") return { label: \"Waiting\", detail: \"Approval required\", kind: \"warning\" };",
    "    const routed = readAssistantStateFromRouteOutcome(routeOutcome);",
    ...
    "  }",
    "",
    "  function readAssistantStateFromRouteOutcome(routeOutcome) {",
    "    switch (routeOutcome.kind) {",
    "      case \"approval_required\":",
    "        return { label: \"Approval\", detail: routeOutcome.title, kind: \"warning\" };",
    "      case \"needs_confirmation\":",
    "        return { label: \"Confirm\", detail: routeOutcome.title, kind: \"warning\" };",
    "      case \"needs_clarification\":",
    "        return { label: \"Clarify\", detail: routeOutcome.title, kind: \"warning\" };",
    "      case \"app_policy_denied\":",
    "        return { label: \"Policy denied\", detail: routeOutcome.title, kind: \"error\" };",
    "      case \"chrome_host_policy_denied\":",
    "        return { label: \"Chrome policy denied\", detail: routeOutcome.title, kind: \"error\" };",
    "      case \"user_denied\":",
    "        return { label: \"Denied\", detail: routeOutcome.title, kind: \"ok\" };",
    "      case \"blocked\":",
    "        return { label: \"Blocked\", detail: routeOutcome.title, kind: \"error\" };",
    "      case \"cancelled\":",
    "        return { label: \"Cancelled\", detail: routeOutcome.title, kind: \"ok\" };",
    "      case \"stopped\":",
    "        return { label: \"Stopped\", detail: routeOutcome.title, kind: \"ok\" };",
    "      case \"failed\":",
    "        return { label: \"Failed\", detail: routeOutcome.title, kind: \"error\" };",
    "      case \"completed\":",
    "        return { label: \"Done\", detail: routeOutcome.title, kind: \"ok\" };",
    "      default:",
    "        return undefined;",
    "    }",
    "  }",
    "",
    "  function readFallbackRouteOutcomeDetail(routeOutcome) {",
    "    return routeOutcome && routeOutcome.kind !== \"idle\" && routeOutcome.kind !== \"unknown\" ? routeOutcome.detail : undefined;",
    "  }",
    "",
    "  function readUserNextAction(snapshot, routeOutcome = readFallbackRouteOutcome(snapshot, snapshot.currentTurn || {})) {",
    "    const alerts = readArray(snapshot.alerts);",
    ...
    "    if (routeNextAction && routeOutcome.kind !== \"approval_required\") return routeNextAction;",
    "    if (turn.approvalState === \"required\") return \"Review the pending approval.\";",
    "    if (routeNextAction) return routeNextAction;",
    "    const extension = snapshot.runtimeHealth && snapshot.runtimeHealth.extension || {};",
    ...
    "  }",
    "",
    "  function readNextActionFromRouteOutcome(routeOutcome) {",
    "    switch (routeOutcome.kind) {",
    "      case \"approval_required\":",
    ...
    "      case \"needs_clarification\":",
    "        return \"Clarify route\";",
    "      case \"app_policy_denied\":",
    "        return \"Review app policy denial\";",
    "      case \"chrome_host_policy_denied\":",
    "        return \"Review Chrome host policy denial\";",
    "      case \"user_denied\":",
    "        return \"Route denied by user\";",
    "      case \"blocked\":",
    "        return \"Resolve route blocker\";",
    "      case \"cancelled\":",
    "        return \"Route cancelled\";",
    "      case \"stopped\":",
    "        return \"Task stopped\";",
    "      case \"failed\":",
    "        return \"Review route failure\";",
    "      case \"completed\":",
    "        return \"Route completed\";",
    "      case \"running\":",
    "        return \"Monitor running route\";",
    "      default:",
    "        return undefined;",
    "    }",
    "  }",
    "",
    "  function readFallbackRouteOutcome(snapshot, turn) {",
    "    const explicit = snapshot.routeOutcome && typeof snapshot.routeOutcome === \"object\" ? snapshot.routeOutcome : {};",
    "    const replay = snapshot.replay && typeof snapshot.replay === \"object\" ? snapshot.replay : {};",
    "    const latestReplayEvent = readLatestReplayRouteEvent(replay);",
    "    const latestToolCall = replay.latestToolCall && typeof replay.latestToolCall === \"object\" ? replay.latestToolCall : {};",
    "    const state = readRouteStateValue(explicit.state) || readRouteStateValue(turn.state) || readRouteStateValue(latestReplayEvent.status) || readRouteStateFromReplayOutcome(replay.outcome || (replay.transcript && replay.transcript.outcome)) || readRouteStateValue(latestToolCall.status) || readRouteStateValue(replay.state) || \"idle\";",
    "    const routeTurn = { ...latestReplayEvent, ...turn, state };",
    "    const detail = explicit.detail || turn.routeReason || turn.reason || turn.latestMessage || turn.message || latestReplayEvent.routeReason || latestReplayEvent.reason || latestReplayEvent.latestMessage || latestReplayEvent.message || readStopTurnMessage(routeTurn) || latestToolCall.summary || latestToolCall.evidenceSummary || turn.command || latestToolCall.command || \"Ready for an agent or Computer Use turn.\";",
    "    const kind = readRouteOutcomeKindValue(explicit.kind) || readRouteOutcomeKindFromTurn(routeTurn, detail);",
    "    return {",
    "      kind,",
    "      title: explicit.title || readFallbackRouteOutcomeTitle(kind),",
    "      value: readRouteStateValue(explicit.value) || explicit.value || (kind === \"running\" ? state : kind),",
    "      detail,",
    "      state",
    "    };",
    "  }",
    "",
    "  function readLatestReplayRouteEvent(replay) {",
    "    const timelineTail = readArray(replay.timelineTail).filter((event) => event && typeof event === \"object\" && !Array.isArray(event));",
    "    const timeline = readArray(replay.timeline).filter((event) => event && typeof event === \"object\" && !Array.isArray(event));",
    "    return timelineTail.at(-1) || timeline.at(-1) || {};",
    "  }",
    "",
    "  function readRouteStateFromReplayOutcome(outcome) {",
    "    return readRouteStateValue(outcome);",
    "  }",
    "",
    "  function readRouteStateValue(value) {",
    "    if (!value || value === \"available\" || value === \"empty\" || value === \"missing\") return undefined;",
    "    if (value === \"approval-required\" || value === \"requires_approval\" || value === \"requires-approval\" || value === \"needs_approval\" || value === \"needs-approval\") return \"approval_required\";",
    "    if (value === \"needs_user_confirmation\" || value === \"needs-user-confirmation\" || value === \"needs-confirmation\") return \"needs_confirmation\";",
    "    if (value === \"needs_user_clarification\" || value === \"needs-user-clarification\" || value === \"needs-clarification\") return \"needs_clarification\";",
    "    if (value === \"verification_failed\") return \"needs_confirmation\";",
    "    if (value === \"passed\" || value === \"verified\" || value === \"ok\" || value === \"success\" || value === \"succeeded\") return \"completed\";",
    "    if (value === \"error\" || value === \"errored\" || value === \"timed_out\" || value === \"timed-out\" || value === \"timeout\") return \"failed\";",
    "    if (value === \"app-policy-denied\" || value === \"app_policy_blocked\" || value === \"app-policy-blocked\" || value === \"blocked_by_app_policy\" || value === \"blocked-by-app-policy\" || value === \"denied_by_app_policy\" || value === \"denied-by-app-policy\") return \"app_policy_denied\";",
    "    if (value === \"chrome-host-policy-denied\" || value === \"chrome_host_policy_blocked\" || value === \"chrome-host-policy-blocked\" || value === \"host_policy_blocked\" || value === \"host-policy-blocked\" || value === \"blocked_by_host_policy\" || value === \"blocked-by-host-policy\" || value === \"blocked_by_chrome_host_policy\" || value === \"blocked-by-chrome-host-policy\" || value === \"denied_by_host_policy\" || value === \"denied-by-host-policy\" || value === \"denied_by_chrome_host_policy\" || value === \"denied-by-chrome-host-policy\") return \"chrome_host_policy_denied\";",
    "    if (value === \"user-denied\" || value === \"denied_by_user\" || value === \"denied-by-user\") return \"user_denied\";",
    "    if (value === \"canceled\") return \"cancelled\";",
    "    return value;",
    "  }",
    "",
    "  function readRouteOutcomeKindValue(value) {",
    "    const state = readRouteStateValue(value);",
    "    if (state === \"denied\") return \"user_denied\";",
    "    if ([\"planned\", \"observing\", \"executing\", \"running\"].includes(state)) return \"running\";",
    "    if ([\"idle\", \"approval_required\", \"needs_confirmation\", \"needs_clarification\", \"app_policy_denied\", \"chrome_host_policy_denied\", \"user_denied\", \"blocked\", \"cancelled\", \"stopped\", \"failed\", \"completed\", \"unknown\"].includes(state)) return state;",
    "    return undefined;",
    "  }",
    "",
    "  function readRouteOutcomeKindFromTurn(turn, detail) {",
    "    const state = turn.state || \"idle\";",
    "    const classifier = [turn.denialKind, turn.policyKind, turn.routeReason, turn.reason, turn.latestMessage, turn.message, detail, turn.updateSource].filter(Boolean).join(\" \").toLowerCase();",
    "    const approvalPending = turn.approvalState === \"required\" || turn.approvalState === \"pending\" || turn.approvalRequired === true;",
    "    if (state === \"approval_required\" || (approvalPending && canApprovalOverrideRouteState(state))) return \"approval_required\";",
    "    if (state === \"needs_confirmation\") return \"needs_confirmation\";",
    "    if (state === \"needs_clarification\") return \"needs_clarification\";",
    "    if (state === \"app_policy_denied\") return \"app_policy_denied\";",
    "    if (state === \"chrome_host_policy_denied\") return \"chrome_host_policy_denied\";",
    "    if (state === \"user_denied\") return \"user_denied\";",
    "    if (isDeniedOrBlockedRouteState(state) && (turn.denialKind === \"app_policy\" || turn.policyKind === \"app-policy\" || classifier.includes(\"denied by app policy\"))) return \"app_policy_denied\";",
    "    if (isDeniedOrBlockedRouteState(state) && (turn.policyKind === \"chrome-host-policy\" || classifier.includes(\"chrome host policy blocked\") || classifier.includes(\"chrome-host-policy\"))) return \"chrome_host_policy_denied\";",
    ...
    "  }",
    "",
    "  function canApprovalOverrideRouteState(state) {",
    "    return ![\"needs_confirmation\", \"needs_clarification\", \"blocked\", \"denied\", \"app_policy_denied\", \"chrome_host_policy_denied\", \"user_denied\", \"cancelled\", \"failed\", \"completed\"].includes(state);",
    "  }",
    "",
    "  function isDeniedOrBlockedRouteState(state) {",
    "    return state === \"denied\" || state === \"blocked\";",
    "  }",
    "",
    "  function readStopTurnMessage(turn) {",
    "    const behavior = turn.stopTurnBehavior && typeof turn.stopTurnBehavior === \"object\" ? turn.stopTurnBehavior : {};",
    "    return behavior.afterMessage || behavior.behaviorAfterMessage;",
    "  }",
    "",
    "  function readFallbackRouteOutcomeTitle(kind) {",
    "    switch (kind) {",
    "      case \"approval_required\": return \"Route approval required\";",
    "      case \"needs_confirmation\": return \"Route needs confirmation\";",
    "      case \"needs_clarification\": return \"Route needs clarification\";",
    "      case \"app_policy_denied\": return \"App policy denied route\";",
    "      case \"chrome_host_policy_denied\": return \"Chrome host policy denied route\";",
    "      case \"user_denied\": return \"User denied route\";",
    "      case \"blocked\": return \"Route blocked\";",
    "      case \"cancelled\": return \"Route cancelled\";",
    "      case \"stopped\": return \"Route stopped\";",
    "      case \"failed\": return \"Route failed\";",
    "      case \"completed\": return \"Route completed\";",
    "      case \"running\": return \"Route running\";",
    "      case \"idle\": return \"No active route\";",
    "      default: return \"Route state unknown\";",
    "    }",
    "  }",
    "",
