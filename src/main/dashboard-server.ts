...
function renderDashboardScript(): string {
  return [
    ...
    "    const kind = liveConnection === \"connected\" && nativeHost.state === \"installed\" ? \"ok\" : nativeHost.state === \"installed\" ? \"warning\" : \"error\";",
    "    setUserRows(\"apps-sites\", [",
    "      row(\"Chrome\", liveConnection === \"connected\" ? \"Connected\" : \"Needs extension refresh\"),",
    "      row(\"Native host\", nativeHost.state || \"unknown\"),",
    "      row(\"Page control\", `${pageControl.capable ? \"ready\" : \"not ready\"}${pageControl.state ? ` (${pageControl.state})` : \"\"}`),",
    "      row(\"Host policy\", extension.hostPolicy && extension.hostPolicy.state),",
    "      row(\"Fallback\", pageControl.capable ? \"DOM control\" : \"screenshot / app control\")",
    "    ], kind === \"ok\" ? \"Ready\" : \"Needs setup\", kind);",
    "  }",
    "",
