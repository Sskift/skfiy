...
function renderDashboardScript(): string {
  return [
    ...
    "    const empty = panel.freshInstall === true || runtimeSnapshot.freshInstall === true || panel.emptyReasonCode || runtimeSnapshot.emptyReasonCode || runtimeSnapshot.state === \"missing\";",
    "    const unavailable = runtimeSnapshot.state === \"unavailable\" || runtimeSnapshot.state === \"repair-failed\" || runtimeSnapshot.state === \"isolated\";",
    "    const stale = panel.stale === true || runtimeSnapshot.stale === true || (ageSeconds !== undefined && ageSeconds > RUNTIME_SNAPSHOT_STALE_SECONDS);",
