...
function renderDashboardScript(): string {
  return [
    ...
    "    const state = explicit.state || turn.state || latestReplayEvent.status || readRouteStateFromReplayOutcome(replay.outcome || (replay.transcript && replay.transcript.outcome)) || readRouteStateValue(replay.state) || \"idle\";",
