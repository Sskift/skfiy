...
function renderDashboardScript(): string {
  return [
    ...
    "    const state = explicit.state || turn.state || \"idle\";",
    "    const detail = explicit.detail || turn.routeReason || turn.reason || turn.latestMessage || turn.message || turn.command || \"Ready for an agent or Computer Use turn.\";",
    "    const kind = explicit.kind || readRouteOutcomeKindFromTurn(turn, detail);",
    "    return {",
    "      kind,",
