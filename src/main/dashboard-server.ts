...
function renderDashboardScript(): string {
  return [
    ...
    "        state: aggregateEvidenceState([chromeRuntimeEvidenceState(extension.state), nativeHostEvidenceState(nativeHost.state), artifactEvidenceState(chromeArtifact), resultEvidenceState(nativeHostBridge.result), installedExtension.result === \"blocked\" ? \"needs-evidence\" : resultEvidenceState(installedExtension.result)]),",
    "        summary: `extension ${extension.state || \"unknown\"}, host ${nativeHost.state || \"unknown\"}, smoke ${chromeArtifact && chromeArtifact.result || \"missing\"}`",
    "      }",
    "    ];",
    "  }",
    "",
