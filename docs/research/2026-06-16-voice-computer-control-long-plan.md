...
- Add Finder proof of concept:
  ...
  - remaining product evidence gap: Finder now counts as dogfood coverage for the safe test-folder organization path only when observe_app and semantic selection both pass or report concrete permission blocks; the next gap is a richer Finder UI adapter that can perform drag/drop and operate on user-chosen folders instead of only a constrained temporary-folder workflow
