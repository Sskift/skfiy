...
- Add Finder proof of concept:
  ...
  - remaining product evidence gap: Finder now counts as dogfood coverage for the safe test-folder organization path, current Finder window grounding path, and selected Finder folder grounding path when observe_app and semantic selection pass or report concrete permission blocks; the next gap is a richer Finder UI adapter that can perform drag/drop and dogfood against real user directories beyond temporary fixtures
