...
1. Finish the Chrome extension action bridge before claiming "browser control": `skfiy chrome observe/screenshot/click/fill/submit/scroll` is now the packaged CLI action surface, `chrome reload-extension` has extension-context self-reload proof, and `chrome tabs` can discover tabs through packaged Apple Events fallback with numeric target ids. The latest installed-extension action smoke can drive a loopback fixture until final text shows `clicked 1 submitted skfiy #2`, and local code now passes CLI-owned `skfiyRequestId` values through page-control wake URLs, but the gate is still blocked because Chrome has the installed extension registered at `0.0.7` while local source is `0.0.8`, reload can fall back into `desktop-session-locked`, screenshot still needs Chrome capture permission or a desktop fallback pass, MV3-native tab discovery still needs fresh `skfiy.tabs.discover` evidence, dashboard controls are not one-click user actions yet, and real per-command evidence correlation must be proven after the extension card is refreshed.
...
5. **Chrome extension evidence:** execute `docs/superpowers/plans/2026-06-21-browser-control-dashboard-iteration.md`. Keep Native Messaging host heartbeat evidence and promote the manually installed branded-Chrome proof into a repeatable smoke. The current P0 proof can reload the extension, discover tabs through packaged Apple Events fallback, observe, fill, click, submit, and scroll an authorized ordinary HTTP page through the packaged CLI. The next two-week work is to close screenshot permission/fallback, finish or product-label MV3 tab discovery, build dashboard launchers, add sensitive-flow gates, and repeat installed-extension evidence without relying on branded Chrome's removed `--load-extension` path.
