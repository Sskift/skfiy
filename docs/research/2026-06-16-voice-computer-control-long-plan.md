...
- [ ] Internal dogfood with 3-5 real users:
  ...
  - [x] tester handoff generator via `npm run dogfood:handoff -- --manifest <path> --tester-id <id> --output <path>`, which writes alpha zip identity, SHA256, no-tmux rules, permission setup, exact tester command, filing instructions, and maintainer review commands without creating reports or accepting evidence
