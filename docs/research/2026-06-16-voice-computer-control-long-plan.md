...
- [ ] Internal dogfood with 3-5 real users:
  ...
  - [x] `dogfood:status` now exposes `readiness.canRunPassedCohort` and a `Passed cohort gate ready` summary line, keeping non-strict report collection readiness separate from the final `dogfood:cohort --require-passed` gate
