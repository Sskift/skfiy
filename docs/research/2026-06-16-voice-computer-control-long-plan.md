...
- Add Finder proof of concept:
  ...
  - remaining product evidence gap: Finder now counts as dogfood coverage for the safe test-folder organization path, current Finder window grounding path, selected Finder folder grounding path, native HID drag-probe, and fixture-level file-icon drag/drop when observe_app, semantic selection, layout, and drag either pass or report concrete permission blocks; the next gap is a richer Finder UI adapter that can dogfood against real user directories beyond temporary fixtures
