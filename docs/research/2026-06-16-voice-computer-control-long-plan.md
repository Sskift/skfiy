...
- [ ] Internal dogfood with 3-5 real users:
  ...
  - [x] `dogfood:tester` now validates any explicit `--app` bundle before running product smokes, rejecting non-`skfiy.app` paths or `Info.plist` identities that are not lowercase `skfiy` with bundle id `com.sskift.skfiy`
