...
- Add Finder proof of concept:
  ...
  - [x] product-path test-folder organization evidence
    - `npm run smoke:finder -- --require-passed --output .skfiy-smoke/finder-organize.json` launches `dist/skfiy.app`, routes the command through renderer -> preload -> main -> helper observe_app -> fs -> Finder, records Finder app policy approval, captures `finderObservation` for Finder focus/screenshot evidence, verifies `create_folder` / `move_file`, and checks before/after trees
  - remaining product evidence gap: Finder now counts as dogfood coverage for the safe test-folder organization path only when observe_app passes or reports a concrete permission block; the next gap is a richer Finder UI adapter that can ground selection, drag/drop, and user-chosen folders instead of only a constrained temporary-folder workflow
