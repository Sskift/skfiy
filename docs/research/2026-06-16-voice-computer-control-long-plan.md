...
- [ ] Internal dogfood with 3-5 real users:
  ...
  - [x] dogfood status now reports tracking-issue workflow checklist coverage for `coding-terminal`, `screenshot-inspection`, `finder-file`, and `browser-fallback`, so maintainers can steer the next real tester before running `dogfood:collect`
