...
- [ ] Internal dogfood with 3-5 real users:
  ...
  - [x] `dogfood:report` reads accepted issue body and labels with `gh issue view` by default, deriving tester/workflow metadata and tester smoke artifact paths from the report issue, and `dogfood:cohort` requires accepted issue label metadata that matches the reported workflows
