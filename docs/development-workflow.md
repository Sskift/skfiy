...
After rerunning `smoke:desktop-session`, pass `--desktop-session-artifact .skfiy-smoke/desktop-session-current.json` into `dogfood:status` so the latest desktop preflight can clear or replace stale loginwindow/display-sleep blockers from older product smoke artifacts.
