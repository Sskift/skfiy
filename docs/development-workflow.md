...
This launches `dist/skfiy.app` via LaunchServices, drags the real pet upward through renderer pointer events, clicks the pet, waits for the React permission onboarding state, and records the visible permission rows. A `passed` result requires `runnerHasTmux=false`, the product path `LaunchServices -> renderer DOM -> React permission onboarding`, `petClicked=true`, `petDrag.result=passed`, `petDrag.beforeBounds`, `petDrag.afterBounds`, `petDrag.totalDeltaY < 0`, `petDrag.suppressedClickAfterDrag=true`, `onboardingVisible=true`, and provider-relevant permission rows in the overlay. For the default external Doubao path, those blocking rows are Screen Recording and Accessibility. If all permissions required by the selected provider are already granted, the expected result is `no-onboarding`.

When onboarding is visible, the UI smoke artifact must also include `permissionSettingTargets` for the provider-relevant permissions. The default external Doubao path needs direct Screen Recording and Accessibility targets; Microphone and Speech Recognition targets are only required when the selected browser or native macOS speech provider needs them. These targets prove the overlay exposes direct macOS System Settings entries rather than a generic help message.
...
The default voice smoke uses `provider: doubao`. skfiy triggers Doubao Input Method externally, then the smoke submits the resulting text through the renderer/preload text bridge. It does not embed Doubao and does not require macOS Speech Recognition permission. Use `npm run smoke:voice -- --require-passed` only after Screen Recording and Accessibility are granted to `dist/skfiy.app`, because the transcript still has to drive real Computer Use. A `passed` result requires LaunchServices app launch, `runnerHasTmux=false`, the product path `renderer -> preload -> main -> external Doubao Input Method -> text bridge -> Computer Use`, `externalInput.source: doubao-input-method`, provider events showing listening and stopped, a final non-empty transcript event, downstream `taskEvents`, and `turnReplay` evidence from `window.skfiy.getTurnReplay()` with a completed Ghostty transcript, `com.mitchellh.ghostty`, `type_text`, `press_key`, passed verification actions, and non-empty before/after screenshot bytes.

For optional native macOS Speech evidence, run:

```bash
npm run smoke:voice -- --provider native-macos --output .skfiy-smoke/voice-native.json
```

Use native `--require-passed` only after Microphone and Speech Recognition are granted to `dist/skfiy.app`. Native artifacts must include structured `speechStatus`; `no-transcript` evidence should include native provider `listening` plus `no_transcript` or `cancelled` events, no final transcript, and no downstream task submission.

### Dogfood Evidence Gate

After creating an alpha manifest, verify that the manifest, zip, UI smoke artifact, Ghostty smoke artifact, Chrome smoke artifact, Finder smoke artifact, and voice smoke artifact form one coherent evidence chain:

```bash
npm run dogfood:verify -- --manifest .skfiy-alpha/skfiy-0.1.0-<commit>-macos-unsigned.json
```

Use `--require-current-head` when validating a local alpha before sharing it; this fails if the manifest was created from an older commit than the current worktree HEAD. Use `--require-passed` only for a release gate after Ghostty, Chrome, Finder, and the selected voice smoke runs are expected to pass. Without `--require-passed`, permission-blocked runs are acceptable evidence only when they still prove the packaged app path, `runnerHasTmux=false`, product path, cleanup, app policy settings, external Doubao voice transcript-to-task and Ghostty turn replay evidence for passed voice runs, external Doubao voice no-transcript/cancellation lifecycle evidence for no-transcript runs, accepted GitHub dogfood issue source evidence, Chrome extraction evidence, Chrome Native Messaging heartbeat evidence, Chrome installed-extension smoke evidence, Chrome current-page observation evidence, Chrome sensitive-page pause evidence, Chrome form action evidence, Chrome screenshot fallback evidence, Chrome fallback switching evidence, Finder observe_app evidence, Finder semantic selection evidence, Finder plan preview evidence, Finder plan confirmation evidence for current/selected folder runs, Finder item drag/drop evidence, Finder organization evidence, clipboard read/write approval runs, and required manifest links.
...
The default mode is a dry-run that writes the issue body locally. Review it, then add `--execute` to run `gh issue edit` for the tracking issue. The generated body preserves existing accepted report issue URLs in the required real-tester slots while refreshing current alpha release/manifest/zip/SHA/commit identity, synthetic local preflight evidence, and the exact status, prepare, tester, review, and cohort commands for that alpha. The generated tracking issue body includes a `Desktop Session Preflight` section that tells testers to run `smoke:desktop-session` before `--require-passed` and to treat locked console, `com.apple.loginwindow`, display sleep, or black-screen evidence as desktop-session blockers. The generated prepare and review commands carry the same `--tracking-issue-url`, so workflow inference and accepted-report tracking stay attached to the intended cohort issue.
...
npm run dogfood:status -- \
  ...
  --json-output .skfiy-dogfood/status.json
```

`dogfood:status` reports local smoke results, permission blockers, accepted report URLs already filled into the tracking issue, the tracking issue `Current Alpha` identity, current git HEAD comparison when available, current-alpha assignment packet comment presence when GitHub comments are readable, and a non-mutating validation of each linked report issue. The tracking issue identity check requires its release tag, manifest, zip, zip SHA256, commit, bundle id, and app name to match the selected manifest before `ready-to-collect` can be reported. It also validates that the tracking issue body still includes `Desktop Session Preflight` and emits a `dogfood:tracking-issue --execute` refresh command when that guidance is missing. If the selected alpha manifest is older than HEAD, status adds a next action to publish a fresh alpha or intentionally keep testing the older selected alpha; this is a warning by default and only becomes a strict collect gate with `--require-current-head`. The linked issue validation checks `dogfood:accepted`, matching alpha manifest/zip/commit identity, at least one checked cohort workflow, and exact `workflow:*` labels before it counts the issue as a verified accepted report. Real tester readiness excludes reserved local synthetic tester ids such as `local-*`, `prepare-*`, `preflight-*`, and `synthetic-*`; those reports remain useful local evidence but do not count toward the 3-5 real-user gate, workflow coverage, or passed workflow coverage. `dogfood:status` writes a `Permission Evidence` section in Markdown and `localSmoke.permissionEvidence` in JSON, preferring the app-scoped `smoke:ui` `permissionDiagnostics.active` source for Speech Recognition when present; direct-helper Speech Recognition readings from `smoke:desktop-session` are diagnostic only and are preserved as non-authoritative helper evidence instead of overriding app-scoped grants. It also writes a `Recommended Tester Assignments` section with copyable `dogfood:prepare-alpha`, `dogfood:tester`, and `dogfood:review` commands for the next real tester slots and missing workflow coverage; when the current assignment packet is already posted, the JSON `nextActions` list repeats those `dogfood:prepare-alpha` commands so dashboards and follow-up agents can dispatch tester preparation without scraping the Markdown summary. When local prepared alpha assets already exist, `dogfood:status` replaces the tester command placeholders with those manifest and app paths and emits direct `dogfood:tester` next actions. Suggested `tester-N` ids skip tester ids already parsed from linked reports, and when status is based on a GitHub tracking issue, the generated review commands carry the same `--tracking-issue-url` so accepted reports are linked back to the intended cohort issue instead of the default. If assignments exist but the tracking issue has no current `skfiy-alpha-<commit>` assignment packet comment with `App Bundle Preflight`, `Desktop Session Preflight`, `Permission Preflight`, and `Evidence Preview Gate`, status adds a next action to post the packet before asking more testers to run it. Once source coverage is complete and only passed workflow coverage is missing, the assignment purpose changes to `passed-workflow-evidence` and the generated prepare/tester commands include `--require-passed`. It does not create reports, edit GitHub, or mark the cohort ready; use it to decide whether the next step is publishing a fresh alpha, updating stale alpha links, posting tester assignments, granting permissions, replacing stale issue links, collecting tester reports, or running `dogfood:collect`. Add `--require-current-head` only when validating a local alpha before publication; published-release status checks must keep working after later documentation-only commits.
When `--json-output` is provided, it writes the same status object to a pure JSON file so follow-up automation can read tester assignments, next actions, current alpha identity, and coverage without scraping npm stdout.
After rerunning `smoke:desktop-session`, pass `--desktop-session-artifact .skfiy-smoke/desktop-session-current.json` into `dogfood:status` so the latest desktop preflight can clear or replace stale loginwindow/display-sleep blockers from older product smoke artifacts. When a desktop blocker is present, `dogfood:status` includes that exact artifact-backed status refresh command in `nextActions`.
...
`dogfood:assignments` is non-mutating by default. It reads the same recommended tester assignments as `dogfood:status`, writes a Markdown packet with prepare/tester/review commands, and does not create or accept reports, add labels, update cohort JSON, or close workflow coverage. When `--json-output` is provided, it also writes the tester split, app bundle preflight, desktop session preflight, permission preflight, evidence preview gate, next actions, and GitHub issue comment command as machine-readable JSON for dashboards and follow-up agents. The default output includes the GitHub issue comment command but does not run it; add `--execute` only when the packet should be posted as a GitHub issue comment on the tracking issue. The packet includes `Packet schema: dogfood-assignments-v2`; `dogfood:status` treats current-alpha assignment comments without that schema as stale so changed tester instructions are reposted before the next dogfood run. The packet includes an `App Bundle Preflight` section that tells testers `dogfood:tester` verifies the selected `skfiy.app` identity, runs `codesign --verify --deep --strict`, and requires `designated => identifier "com.sskift.skfiy"` before product smokes, so permission rows are interpreted against the same extracted app identity that will run. It also includes a `Desktop Session Preflight` section that surfaces locked console, `com.apple.loginwindow`, and display-sleep blockers from the selected smoke evidence, plus a `Permission Preflight` section that lists Screen Recording, Accessibility, Microphone, and Speech Recognition states. Testers should use `--require-passed` only after `smoke:desktop-session` passes on their machine and the provider-relevant permissions are granted to the extracted `skfiy.app`; for the default external Doubao path, that means Screen Recording and Accessibility. Otherwise they should run the normal tester command and file blocked evidence. The packet also includes an `Evidence Preview Gate` section that tells testers to confirm `reportPreviewEligibility.eligible=true` from `dogfood:issue -- --check-report` before filing and to preserve blocking checks when the preview is false. It explicitly calls out UI pet drag evidence from the packaged app, including renderer pointer events, before/after bounds, upward movement, and suppressed click-after-drag. It also calls out panic stop evidence from `runtimeStatus.stopTurnHotkey` plus UI smoke `stopTurnBehavior`, including `behaviorResult: passed`, `behaviorSource: renderer-escape-key-product-path`, `behaviorBeforeStatus: approval_required`, `behaviorAfterStatus: idle`, and `behaviorAfterMessage: Task stopped.`.
...
`dogfood:prepare-alpha` downloads the release zip and manifest, verifies the zip SHA256 against the manifest, rejects manifests missing long-horizon money-run evidence or `Panic stop product-path behavior evidence`, extracts `skfiy.app`, installs or reuses the prepared app under `.skfiy-dogfood/apps/<tag>/`, and creates a handoff whose `dogfood:tester` command points at that prepared app bundle. Existing app destinations are identity-checked and reused by default so multiple tester handoffs for the same alpha do not require destructive reinstalls; the shared app install path is guarded by `.install.lock`, so parallel tester preparations wait and re-check the completed bundle instead of running concurrent `ditto` installs into the same `.app`. Pass `--replace-existing` only when intentionally overwriting the destination app. It defaults to dry-run; `--execute` is required before it downloads or writes local files. If `--workflows` is omitted but `--tracking-issue-url` or `--tracking-issue-file` is provided, it reads `Recommended Tester Assignments` from the tracking issue body and, for GitHub URLs, the current alpha assignment packet comments with `App Bundle Preflight`, `Desktop Session Preflight`, `Permission Preflight`, and `Evidence Preview Gate`; it then passes the tester's workflows into the generated handoff. Add `--require-passed` only for tester machines with `smoke:desktop-session` passing and provider-relevant permissions ready; it propagates to the handoff and `nextCommands.tester`.
...
This preflight is non-mutating by default. It checks the filed issue body and artifact paths through the same manifest-backed parser used by `dogfood:report`, then prints suggested labels, a copy-safe `gh issue edit ... --add-label ...` acceptance command, and a `dogfood:tracking-issue --accepted-report-url ...` command for eligible real tester reports. If alpha identity or artifact validation fails, the command still exits failed but writes a `Result: rejected` summary with the blocking reason and without acceptance/tracking commands, so maintainers can send the checklist back to the tester. After checking the summary, maintainers may rerun `dogfood:review` with `--execute`; it adds the missing acceptance/workflow labels and refreshes the tracking issue in one validated step. Synthetic tester ids do not get a tracking-slot command.
...
The draft fills the `###` sections parsed by `dogfood:report`, including alpha identity, all five smoke artifact paths, app bundle preflight (`appPath`, LaunchServices launch command, `appLaunchViaOpen`, `runnerHasTmux`, and product path), UI pet drag evidence (`result`, `source`, `beforeBounds`, `afterBounds`, `moveEvents`, `totalDeltaX`, `totalDeltaY`, `upwardMovement`, and `suppressedClickAfterDrag`), panic stop evidence from `runtimeStatus.stopTurnHotkey` plus UI smoke `stopTurnBehavior` (`behaviorResult`, `behaviorSource`, `behaviorBeforeStatus`, `behaviorAfterStatus`, and `behaviorAfterMessage`), permission states, ASR provider, Computer Use result, screenshots, action verification messages, and core Chrome/Finder/voice evidence. `--check-report` round-trips the generated draft through the `dogfood:report` parser with synthetic accepted labels, then prints `reportPreviewEligibility` from the `dogfood:cohort` report-level checks. Tester-side artifacts fail locally if a heading, manifest identity, app bundle identity, UI pet drag identity, panic stop identity, or artifact path is incompatible; `reportPreviewEligibility.eligible=false` means the filed issue would not count toward cohort coverage until the listed blocking checks are fixed. After the drafted single-user report is reviewed and accepted, generate its cohort JSON from the accepted issue body:
...
`dogfood:collect` only discovers accepted report issue URLs from the tracking issue's `Required Real Tester Count` slots and then reuses the same manifest-backed `dogfood:report` parser for each issue. It still requires `dogfood:accepted`, matching `workflow:*` labels, issue alpha manifest/zip/commit identity, all five smoke artifact paths, smoke JSON `artifactPath` identity, UI pet drag evidence matching the UI smoke artifact, and panic stop evidence matching `runtimeStatus.stopTurnHotkey` plus `stopTurnBehavior`. It immediately runs `dogfood:cohort`; a failed cohort verification keeps the command from being release evidence. Local synthetic reports may remain in local evidence files, but they cannot satisfy workflow coverage.
...
Chrome native host heartbeat: nativeHostBridgeRun.result passed, dist/skfiy -> Chrome Native Messaging heartbeat
Chrome installed extension: installedExtensionRun.result passed or blocked, installedExtensionRun.productPath Chrome MV3 extension -> Native Messaging -> dist/skfiy heartbeat, recommendedBrowser Chrome for Testing or Chromium when blocked
