...
This preflight is non-mutating. It checks the filed issue body and artifact paths through the same manifest-backed parser used by `dogfood:report`, then prints suggested labels, a copy-safe `gh issue edit ... --add-label ...` acceptance command, and a `dogfood:tracking-issue --accepted-report-url ...` command for eligible real tester reports. If alpha identity or artifact validation fails, the command still exits failed but writes a `Result: rejected` summary with the blocking reason and without acceptance/tracking commands, so maintainers can send the checklist back to the tester. Maintainers still apply labels manually after review and review the generated tracking issue body before adding `--execute`. Synthetic tester ids do not get a tracking-slot command.
