...
`dogfood:prepare-alpha` defaults to dry-run. With `--execute`, it downloads the GitHub release zip and manifest, verifies the zip SHA256 against the manifest, rejects stale manifests that do not require long-horizon money-run supervision or `Panic stop product-path behavior evidence`, extracts `skfiy.app`, checks the extracted `Info.plist` identity (`CFBundleIdentifier=com.sskift.skfiy`, `CFBundleName=skfiy`, `CFBundleDisplayName=skfiy`, `CFBundleExecutable=skfiy`), installs or reuses the app under `.skfiy-dogfood/apps/<tag>/`, and generates a handoff whose `dogfood:tester` command uses that prepared app bundle. If the destination app already exists, the command validates its identity and reuses it so multiple tester handoffs for the same alpha can be prepared without destructive reinstalling. Passing `--tracking-issue-url` lets it infer the tester's assigned workflows from the tracking issue body or the current alpha assignment packet comments with `Desktop Session Preflight`, `Permission Preflight`, and `Evidence Preview Gate`, and keeps the handoff's maintainer review command linked to the same cohort issue. Pass `--app /Applications/skfiy.app --replace-existing` only when the tester intentionally wants to install over an existing Applications copy. The JSON result always includes `nextCommands.tester` with the downloaded manifest path and installed app path filled in; real tester preparations also include `nextCommands.review`, while `--allow-synthetic-tester-id` maintainer preflights stay local-only and expose no review command. `dogfood:tracking-issue` and `dogfood:status` tell testers to copy those commands after preparation instead of guessing an install path. The recommended tester command includes `--file-issue`, which creates only the dogfood report issue after local validation; maintainers still review and accept it separately. Pass `--require-passed` to `dogfood:prepare-alpha` only when the tester machine already has `smoke:desktop-session` passing and provider-relevant permissions ready: Screen Recording and Accessibility for default external Doubao + Computer Use evidence, plus Microphone and Speech Recognition for optional native/browser speech provider tests. It propagates strict mode into the handoff and `nextCommands.tester`.
