...
- After tests pass and the app is releasable, launch the compiled `skfiy.app` outside tmux and use skfiy to supervise the existing `tmux` session `money-run` agent through a real long-horizon task, preserving screenshots, action verification, stop/approval events, and failure-recovery notes as sustained Computer Use evidence.
