...
- `chrome-page-control-not-ready`: the Chrome extension id is inferred from heartbeat evidence, the Native Messaging host is verified as installed, CLI/Dashboard/Browser Context no longer leak raw `allow_host` protocol actions, and a fresh installed-extension heartbeat is connected. The current active `mew.bytedance.net` tab is no longer blocked by skfiy host policy after current-turn allow; it remains blocked by missing Chrome optional host permission for `https://mew.bytedance.net/*` and missing visible-tab capture permission for `<all_urls>`. Do not call live webpage-context answering complete until the Chrome permissions are granted and a fresh page observation becomes ready.
- `desktop-session-asleep`: latest `./dist/skfiy status --json` reports `desktopSession.state: blocked`, `frontmostBundleId: com.apple.loginwindow`, and `mainDisplayAsleep: true`, so live browser permission granting or packaged visual smoke cannot be performed until the desktop session is awake and controllable again.
