...
- The same fresh `./dist/skfiy status --json` reports Chrome readiness as `needs-action`: the Native Messaging host is installed, but the latest heartbeat is stale and current pageControl remains `blocked_by_host_policy` on `mew.bytedance.net` with missing optional Chrome host permission for `https://mew.bytedance.net/*` and missing capture permission for `<all_urls>`.
...
- `chrome-page-control-not-ready`: the Chrome extension id is now inferred from heartbeat evidence, the Native Messaging host is verified as installed, and CLI/Dashboard/Browser Context no longer leak raw `allow_host` protocol actions. Packaged CLI smoke still proves a ready `pageObservation` reaches the real Background Agent prompt, but the live installed extension heartbeat is stale and the currently active Chrome tab remains blocked by host policy plus missing optional Chrome host/capture permissions. Do not call live webpage-context answering complete until a fresh heartbeat is observed for the active page, host policy allows the target host, and optional Chrome permissions are granted for the active page.
