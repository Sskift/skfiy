...
- `chrome-page-control-not-ready`: the Chrome extension id is inferred from heartbeat evidence, the Native Messaging host is verified as installed, CLI/Dashboard/Browser Context no longer leak raw `allow_host` protocol actions, and a fresh installed-extension heartbeat is connected. Live pageControl is still per-current-tab: Mew hosts have been current-turn allowed, but the current active Chrome tab can move to another host such as `bytedance.larkoffice.com`, which must be explicitly allowed before page context can be read. Even after skfiy host policy allows a host, Chrome optional host permission for that origin and visible-tab capture permission for `<all_urls>` must be granted through a user gesture. Do not call live webpage-context answering complete until the active tab's host policy and Chrome permissions are granted and a fresh page observation becomes ready.
