...
- `chrome-page-control-not-ready`: the Chrome extension id is now inferred from heartbeat evidence and the Native Messaging host is verified as installed. Packaged CLI smoke still proves a ready `pageObservation` reaches the real Background Agent prompt, but the live installed extension heartbeat is stale and the currently active Chrome tab remains blocked by host policy plus missing optional Chrome host/capture permissions. Do not call live webpage-context answering complete until a fresh heartbeat is observed for the active page, host policy allows the target host, and optional Chrome permissions are granted for the active page.
