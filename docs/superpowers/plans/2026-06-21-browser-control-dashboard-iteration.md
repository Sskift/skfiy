...
- 2026-06-22 pet-skin update: the renderer pet art is now manifest-driven and separate from backend Computer Use state. The packaged app first asks the main process for local `luoxiaohei-local` under `~/Library/Application Support/skfiy/skins/luoxiaohei-local/skin.pet.json`; if that user-owned origin skin is missing or invalid, it falls back to bundled `skfiy-black-cat`, while `skfiy-cloudbot` remains a legacy built-in. `skfiy skin import --source <png|gif|webp|svg|jpg> --slug luoxiaohei-local --license-source <source>` now copies authorized local art into the user skin directory and writes a local-only single-frame manifest. Luo Xiaohei official art is the visual reference direction for local prototype skins, but official/third-party images must stay in local licensed skin packs and must not be committed or included in release artifacts without redistribution rights.
...
6. **Pet skin productization**: the first CLI importer exists for single origin images. Next, add a user-facing skin picker/importer, read local skin packs from `~/Library/Application Support/skfiy/skins/`, validate `.pet.json` manifests, crop/pack licensed avatar or sticker art into the 8-by-9 atlas shape, and keep release packaging fail-closed for third-party assets.
