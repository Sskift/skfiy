...
- Dashboard parity: React now renders app readiness lanes for Chrome, Finder,
  ...
  accepting a bare shell, and it treats missing fresh Chrome smoke as typed
  `needs-evidence` Dashboard evidence instead of failing the Dashboard product
  path by forcing a focus-stealing Chrome run. Chrome page actions and
  host-policy controls are now available in the React dashboard.
  Installed-extension execution with the real Chromium id now has a passed local
  action smoke; dashboard and Chrome smokes still both need to stay in the
  release gate so the operator surface and CLI proof cannot drift apart.
