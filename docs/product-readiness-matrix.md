...
- Chrome smoke passes through strict app-policy approval, CDP extraction,
  ...
  Chromium extension id `plcpkkhlcacihjfohlojdknnkademlno`; Native Messaging and
  page-control readiness were present, but the action sequence still failed
  under host-policy/target-tab blockers and is not passed capability proof.
...
- Dashboard parity: React now renders app readiness lanes for Chrome, Finder,
  ...
  available in the React dashboard; installed-extension execution with a real
  Chromium id has been attempted, but the action sequence still needs a passed
  smoke before it counts as real action proof.
...
- Real Computer Use: pet drag/click/stop and Chrome CDP/native-host paths have
  ...
  and generic visible-app fallback remains product-design incomplete rather than
  a ready product route.
...
| P0 | Dashboard and settings | React now owns provider, dogfood/release, replay, Chrome/Finder/Ghostty readiness, Chrome page action controls, and Chrome host-policy controls. Real Chromium extension id `plcpkkhlcacihjfohlojdknnkademlno` proves installed-extension Native Messaging, but installed-extension action execution still fails under host-policy/target-tab blockers. | Fix the installed-extension action path so the selected target tab, host-policy state, and page actions all pass in one smoke artifact, while keeping React controls as the primary operator surface. | `npx vitest run src/dashboard/DashboardApp.test.tsx src/main/dashboard-data.test.ts src/main/dashboard-server.test.ts src/main/dashboard-status.test.ts src/main/dashboard-evidence-summary.test.ts src/main/dashboard-smoke-script.test.ts --reporter=dot` and `npm run smoke:dashboard -- --cli dist/skfiy --require-passed --output .skfiy-smoke/dashboard-<commit>.json` |
...
| P1 | Browser bridge | Chrome CDP/native-host evidence passes, and installed-extension Native Messaging passes with a real Chromium id, but installed-extension action smoke still fails under host-policy/target-tab blockers and screenshot fallback is not a complete action path. | Make extension action proof and screenshot fallback completion part of the Chrome smoke/readiness contract. | `npm run smoke:chrome -- --app dist/skfiy.app --extension-chrome-app "Chromium" --extension-id <id> --require-passed --output .skfiy-smoke/chrome-<commit>.json` |
| P1 | Agent and routing | Route states still lack a first-class `confirmation` route outcome separate from `needs_clarification`; app-policy and user denial now surface as blocked/denied task states instead of generic failure or idle. | Extend route/status contracts so clarification, confirmation, denial, blocked, and failure are distinct at the route boundary too. | `npx vitest run src/main/task-routing.test.ts src/main/app-policy-settings.test.ts src/main/task-status-contract.test.ts --reporter=dot` |
